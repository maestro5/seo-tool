...
RSpec.describe SeReport do
  ...
  describe 'class methods' do
    ...
    it '.all' do
      expect(SeReport).to respond_to(:all)
      expect(SeReport.all).to match_array([])
    end

    it '.count' do
      expect(SeReport).to respond_to(:count)
      expect(SeReport.count).to eq 0
    end

    it '.create' do
      expect(SeReport).to respond_to(:create).with(1).argument
      expect { user.se_reports.create(page) }.to change(SeReport, :count).by(+1)
    end

    it '.find' do
      report_id = user.se_reports.create(page).id
      expect(SeReport).to respond_to(:find).with(1).argument
      expect(SeReport.find(report_id).url).to eq page[:url]
    end

    it '.first' do
      expect(SeReport).to respond_to(:first)
      expect(SeReport.first.url).to eq page[:url]
    end

    it '.last' do
      expect(SeReport).to respond_to(:last)
      expect(SeReport.last.url).to eq page[:url]
    end
  end # class methods
  ...
  describe 'object methods' do
    ...
    let(:report) { user.se_reports.create(page) }
    ...
    it '#destroy' do
      ...
      expect { report.destroy }.to change(user.se_reports, :count).by(-1)
    end
  end # object methods
end # SeReport
